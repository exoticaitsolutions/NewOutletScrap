...
#         self.output_file = BaseScraper._csv_file
...
#                 # Then visit each link and extract date
...
#             self.logger.info(f"Scraped {len(rows)} new headlines from LAcity and saved to {self.output_file}.")
...
class LacityScraper(BaseScraper):
    ...
    def scrape(self):
        ...
        driver = get_chrome_driver()
        seen_urls = self.load_existing_urls()  # from BaseScraper
        all_rows = []
        ...
        try:
            for i, keyword in enumerate(keywords):
                ...
                articles = driver.find_elements(By.CSS_SELECTOR, 'div.search-results > div')
                ...
                for article in articles:
                    try:
                        ...
                        if link in seen_urls:
                            self.logger.info(f"Skipping existing article: {link}")
                            continue
